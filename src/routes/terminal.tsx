      ...
      script.onload = () => {
        const win = window as unknown as {
          TradingView: Record<string, unknown> & {
            widget: new (args: Record<string, unknown>) => void;
          };
        };
        if (typeof win.TradingView !== "undefined") {
...
function TerminalPage() {
  ...
  const [fundingHistory, setFundingHistory] = useState<Record<string, unknown>[]>(() => {
    ...
    const updateFunding = async () => {
      try {
        const res = await fetch(
          `https://fapi.binance.com/fapi/v1/premiumIndex?symbol=${selectedPair.toUpperCase()}USDT`,
        );
        ...
        console.error("Failed to fetch premiumIndex", e);
      }
    };
  ...
  useEffect(() => {
    ...
    const startFallback = () => {
      ...
      const fetchPrice = async () => {
        try {
          const res = await fetch(
            `https://api.binance.com/api/v3/ticker/price?symbol=${selectedPair.toUpperCase()}USDT`,
          );
          const data = await res.json();
          ...
          console.error("Failed to fetch ticker price", e);
        }
      };
  ...
  const handleTrade = () => {
    ...
    openPosition(selectedPair, side, qty, leverage, currentPrice);
          ...
          <div className="flex items-center gap-6">
            ...
            >
              <div className="size-8 rounded-full bg-primary/20 flex items-center justify-center font-bold text-primary overflow-hidden">
                <img
                  src={
                    TOP_COINS.find((c) => c.symbol === selectedPair)?.logo ||
              ...
              <div>
                <div className="flex items-center gap-1.5 font-black text-sm uppercase tracking-tight">
                  {selectedPair}USDT
                  <ChevronDown
                    className={`size-3 text-muted-foreground transition-transform ${isMarketSelectorOpen ? "rotate-180" : ""}`}
                  />
                </div>
            ...
            <div className="flex gap-8">
              <div>
                <div className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mb-0.5">
                  Giá
                </div>
                <div className="text-sm font-black tabular-nums text-success">
              ...
              <div className="hidden sm:block">
                ...
                <div className="text-sm font-black tabular-nums">
                  ${(currentPrice * 0.96).toLocaleString()}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:flex items-center gap-4 text-[10px] font-black uppercase tracking-tighter mr-2">
              <div className="text-muted-foreground">
                Tỷ lệ Funding: <span className="text-amber-500">{fundingRate}</span>
              </div>
              <div className="text-muted-foreground">
                Thời gian: <span className="text-foreground">{fundingTimer}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1 ml-2">
              <button
                onClick={() => navigate({ to: "/" })}
                className="p-2 hover:bg-secondary text-muted-foreground hover:text-foreground rounded-lg transition-all"
                title="Quay lại Ví"
              >
                <X className="size-4" />
              </button>
        ...
        <div className="flex flex-col lg:flex-row min-h-[calc(100vh-56px)] p-2 gap-2">
                    ...
                    {positions
                      ...
                      .map((p) => {
                        ...
                        return (
                          <div
                            key={p.id}
                            className="flex items-stretch rounded-[4px] overflow-hidden shadow-xl border border-white/10"
                          >
                            {/* Color indicator */}
                            ...
                            <div className="px-2 py-1 bg-[#1a1d23] text-[10px] font-black text-white flex items-center gap-1 border-r border-white/10">
                              {p.side === "long" ? (
                                <TrendingUp className="size-3" style={{ color: lineColor }} />
                              ) : (
                                <TrendingDown className="size-3" style={{ color: lineColor }} />
                              )}
                            ...
                            {/* PnL */}
                            <div
                              className={`px-2 py-1 bg-[#1a1d23] text-[10px] font-black tabular-nums flex items-center gap-1 border-r border-white/10 ${pnl >= 0 ? "text-success" : "text-destructive"}`}
                            >
                              {pnl >= 0 ? "+" : ""}
                ...
                <div className="flex items-center gap-6">
                  {[
                    ...
                    { id: "Lịch sử đặt lệnh", label: `Lịch sử đặt lệnh (${orderHistory.length})` },
                    {
                      ...
                      label: `Lịch sử giao dịch (${tradeHistory.length})`,
                    },
                    ...
                    <button
                      ...
                      className={`text-[11px] font-black py-3 transition-all relative ${
                        bottomTab === tab.id
                          ? "text-primary"
              ...
              <div className="flex-1 overflow-x-auto scrollbar-hide">
                {bottomTab === "Vị thế" ? (
                    ...
                    <tbody className="divide-y divide-white/5">
                      {(() => {
                        ...
                        if (filtered.length === 0) {
                          return (
                            <tr>
                              <td
                                colSpan={8}
                                className="px-4 py-10 text-center text-muted-foreground text-[11px] italic"
                              >
                        ...
                        return filtered.map((p) => {
                          const pSymOnly = p.symbol.includes("/")
                            ? p.symbol.split("/")[0]
                            : p.symbol;
                          // CRITICAL: Prioritize currentPrice if it's the active chart asset to ensure 100% sync
                          const pMarkPrice =
                            pSymOnly.toUpperCase() === selectedPair.toUpperCase()
                              ? currentPrice
                              : prices[pSymOnly]?.priceUsd || p.entryPrice;

                          const pnl =
                            p.side === "long"
                              ? (pMarkPrice - p.entryPrice) * p.amount
                              : (p.entryPrice - pMarkPrice) * p.amount;
                          ...
                          return (
                            <tr
                              key={p.id}
                              className="group hover:bg-white/[0.03] transition-colors relative whitespace-nowrap"
                            >
                              <td
                              ...
                              >
                                ...
                                <div
                                  className={`absolute left-0 top-0 bottom-0 w-[3px] ${p.side === "long" ? "bg-success" : "bg-destructive"}`}
                                />
                                ...
                                <div className="flex flex-col ml-1">
                                  ...
                                  <div className="flex items-center gap-1 mt-1">
                                    <span
                                      className={`px-1 rounded-[2px] text-[9px] font-black uppercase ${p.side === "long" ? "bg-success/20 text-success" : "bg-destructive/20 text-destructive"}`}
                                    >
                                      {p.side === "long" ? "Long" : "Short"}
                                    </span>
                                    <span className="px-1 rounded-[2px] bg-white/10 text-muted-foreground/60 text-[9px] font-black uppercase">
                                      {p.leverage}X
                                    </span>
                                  </div>
                                </div>
                              ...
                              <td className="px-4 py-3 text-[12px] font-bold tabular-nums text-foreground">
                                {qtyUnit === "asset"
                                  ? p.amount.toLocaleString()
                                  : (p.amount * pMarkPrice).toLocaleString(undefined, {
                                      maximumFractionDigits: 2,
                                    }) + " USDT"}
                              </td>
                              ...
                              <td className="px-4 py-3">
                                <div className="flex flex-col">
                                  <div
                                    className={`text-[12px] font-bold tabular-nums ${pnl >= 0 ? "text-success" : "text-destructive"}`}
                                  >
                                    {pnl >= 0 ? "+" : ""}
                                    {pnl.toLocaleString(undefined, {
                                      minimumFractionDigits: 2,
                                  ...
                                  <div
                                    className={`text-[10px] font-bold tabular-nums ${pnl >= 0 ? "text-success" : "text-destructive"}`}
                                  >
                                    ({pnl >= 0 ? "+" : ""}
                                    {pnlPercent.toFixed(2)}%)
                                  </div>
                      ...
                      {orderHistory.length === 0 ? (
                        ...
                        orderHistory.map((o) => (
                          <tr key={o.id} className="text-[11px] font-bold hover:bg-white/[0.02]">
                            ...
                            <td className="px-4 py-3">
                              <span
                                className={o.side === "long" ? "text-success" : "text-destructive"}
                              >
                                {o.side.toUpperCase()}
                              </span>
                            </td>
                      ...
                      {tradeHistory.length === 0 ? (
                        ...
                        tradeHistory.map((t) => (
                          <tr key={t.id} className="text-[11px] font-bold hover:bg-white/[0.02]">
                            ...
                            <td
                              className={`px-4 py-3 tabular-nums ${t.pnl >= 0 ? "text-success" : "text-destructive"}`}
                            >
                      ...
                      {fundingHistory.length === 0 ? (
                        ...
                        fundingHistory.map((f) => (
                          <tr key={f.id} className="text-[11px] font-bold hover:bg-white/[0.02]">
                            ...
                            <td
                              className={`px-4 py-3 tabular-nums ${f.amount >= 0 ? "text-success" : "text-destructive"}`}
                            >
                              {f.amount.toFixed(2)}
                            </td>
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-muted-foreground/40">
                    <div className="text-[11px] font-black uppercase tracking-widest italic mb-2">
                      No Data Available
                    </div>
          ...
          <div className="w-full lg:w-[320px] shrink-0 sticky top-[72px] self-start flex flex-col gap-2 pb-10">
                      ...
                      <span className="text-[10px] font-black uppercase text-muted-foreground">
                        ...
                        {(() => {
                          ...
                          return max.toLocaleString(undefined, {
                            maximumFractionDigits: qtyUnit === "asset" ? 3 : 2,
                          });
                        })()}
                      </span>
                    </div>
            ...
            <div className="bg-[#161A1E] rounded-xl border border-white/5 p-4 shadow-xl shrink-0">
              ...
              <div className="space-y-4">
                <div>
                  <div className="text-[10px] text-muted-foreground font-bold uppercase mb-2">
                    Thông tin Ký quỹ
                  </div>
                  <div className="space-y-2.5">
                    <div className="flex justify-between text-[11px]">
                      ...
                      <span className="text-success flex items-center gap-1 font-bold">
                        <ShieldCheck className="size-3" /> An toàn
                      </span>
                    </div>
                    <div className="flex justify-between text-[11px]">
                    ...
                    <div className="flex justify-between text-[11px]">
                      ...
                      <span className="font-bold tabular-nums text-primary">
                        {tradingBalance.toLocaleString()} USDT
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <button className="w-full py-2 bg-secondary/50 hover:bg-secondary text-[11px] font-bold rounded-lg transition-all mt-2">
                  Phân tích Lãi Lỗ hợp đồng
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="absolute top-14 left-4 z-[150] w-[450px] bg-[#161A1E] border border-white/10 rounded-b-2xl shadow-[0_30px_60px_rgba(0,0,0,0.6)] flex flex-col overflow-hidden animate-in slide-in-from-top-2 duration-200">
              ...
              <div className="flex gap-4 px-4 border-b border-white/5 bg-card/5">
                {["Tuyển chọn", "Hợp đồng", "TradFi"].map((tab, idx) => (
                  <button
                    key={tab}
                    className={`text-[11px] font-black py-3 relative ${idx === 0 ? "text-primary" : "text-muted-foreground hover:text-foreground"}`}
                  >
                    {tab}
                    {idx === 0 && (
              ...
              <div className="grid grid-cols-[1fr_80px_80px] gap-2 px-4 py-2 text-[9px] font-black text-muted-foreground uppercase tracking-widest border-b border-white/5 bg-secondary/5">
                <div className="flex items-center gap-1 cursor-pointer hover:text-foreground">
                  Cặp giao dịch / KL <ChevronDown className="size-2" />
                </div>
                <div className="text-right flex items-center justify-end gap-1 cursor-pointer hover:text-foreground">
                  Giá cuối <ChevronDown className="size-2" />
                </div>
                <div className="text-right flex items-center justify-end gap-1 cursor-pointer hover:text-foreground">
                  T.đổi 24h <ChevronDown className="size-2" />
                </div>
              </div>
                ...
                <div className="grid grid-cols-1">
                  {TOP_COINS.filter(
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="text-left">
                            <div className="text-xs font-black uppercase text-foreground group-hover:text-primary transition-colors">
                              {coin.symbol}USDT
                            </div>
                      ...
                      <div
                        className={`text-right text-[10px] font-black ${coin.symbol === "ETH" ? "text-destructive" : "text-success"}`}
                      >
